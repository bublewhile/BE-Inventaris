    ...
    createReturn: async (req, res) => {
        try {
            ...
            const data = {
                ...
                // karena notes tidk wajib di isi, jadi jika kosong definisikan sebagai "-"
                notes: notes ?? "-",
                date: new Date(date)
            }
            ...
            const loan = await Loan.findByPk(loan_id, {include: Item}); //sekaligus mengambil data item nya untuk digunakan stocknya akan di update (+)
            //kalau data peminjaman gada
            if (!loan) {
            ...
            const returnData = await Return.create(data); // value untuk return sudah ada d const data semuanya 
