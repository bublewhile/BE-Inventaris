...
module.exports = {
    createLoan: async (req, res) => {
        try {
            ...
            // ambil data irem sesuai item_id, pastikan id ada di table items
            const item = await Item.findByPk(item_id);
            if (!item) {
                return res.status(400).json(response(400, "Data item not found. please check [item_id] value"));
            }
            // memastikan data total_item yang dipinjam di validasi, kurang dari stok. gabole pinjam lebi dari stok
            if (data.total_item > item.stock) {
                return res.status(400).json(response(400, `Stok not available. Available only ${item.stock}`)); // pake backtick biar bisa ngakses item.stock
            }

            // pake (data) karna dia semua datanya uda komplit, kalo yang di item itu kurang image mkannya pagggilnya stu satu
            const loan = await Loan.create(data);
            // update stok di iitem, kurangi jumlah pinjam
            const updatedStock = await Item.update({
    ...
    getLoans: async (req, res) => {
        try  {
            ...
            // page : ambil data di halaman ke berappa, limit : munculin data beberapa
            // offset : menentukan data yang dimunculkan mulai dari berapa
            const offset = (Number(page)-1) * Number(limit);
            // contoh : page 1 : 1-1 =0 : limitnya 10 : 0 * 10 = 0 jadi offset = 0 jadi datanya mulai dari 1, halaman ke 1 datanya 1-10
            // contoh : page 2 : 2-1 =1 : limitnya 10 : 1 * 10 = 10 jadi offset = 10 datanya mulai dari 1, halaman ke 2 datanya 11-20
            ...
            const formatPagination = {
                ...
                limit: limit,
                // minculin angka 1-20 atau 21-30 sesuai yang diambil. misal offset 20 maka (20-1) (20-10) : 21:30
